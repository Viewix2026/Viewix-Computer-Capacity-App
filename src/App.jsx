...
export default function App(){
  ...
  const rosterAdd=()=>{if(!rosterNewName.trim())return;setEditors(prev=>[...prev,{id:`ed-${Date.now()}`,name:rosterNewName.trim(),phone:"",email:"",role:"editor",defaultDays:{mon:true,tue:true,wed:true,thu:true,fri:true}}]);setRosterNewName("");setRosterAdding(false);};
  ...
  return(<div style={{fontFamily:"'DM Sans',-apple-system,sans-serif",background:"var(--bg)",color:"var(--fg)",minHeight:"100vh",display:"flex"}}><style>{CSS}</style>
    ...
    {tool==="capacity"&&isFounder&&(<>
      ...
      {capTab==="roster"&&(<div style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:12,padding:"20px 24px"}}><div style={{marginBottom:4,fontSize:17,fontWeight:800,color:"var(--fg)"}}>Team Roster</div><div style={{fontSize:12,color:"var(--muted)",marginBottom:20}}>Default working days for all future weeks. Override specific weeks in Weekly Schedule. "Crew" members won't appear in the weekly schedule.</div><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"separate",borderSpacing:0,fontSize:13}}><thead><tr><th style={{...TH,width:150,textAlign:"left"}}>Name</th><th style={{...TH,width:80,textAlign:"center"}}>Role</th><th style={{...TH,width:130,textAlign:"left"}}>Phone</th><th style={{...TH,width:170,textAlign:"left"}}>Email</th>{DL.map(d=>(<th key={d} style={{...TH,textAlign:"center",minWidth:60}}>{d}</th>))}<th style={{...TH,width:45,textAlign:"center"}}>Days</th><th style={{...TH,width:40}}></th></tr></thead><tbody>{editors.map(ed=>{const dn=DK.filter(d=>ed.defaultDays[d]).length;const isE=rosterEditId===ed.id;const edRole=ed.role||"editor";return(<tr key={ed.id} style={{opacity:edRole==="crew"?0.75:1}}><td style={{...TD,fontWeight:700,color:"var(--fg)",cursor:"pointer"}} onClick={()=>{if(!isE){setRosterEditId(ed.id);setRosterEditName(ed.name);}}}>{isE?(<input ref={rosterEditRef} type="text" value={rosterEditName} onChange={e=>setRosterEditName(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")rosterRename();if(e.key==="Escape")setRosterEditId(null);}} onBlur={rosterRename} style={{width:"100%",padding:"3px 6px",borderRadius:4,border:"1px solid var(--accent)",background:"var(--input-bg)",color:"var(--fg)",fontSize:13,fontWeight:700,outline:"none"}}/>):(<span style={{borderBottom:"1px dashed #3A4558"}}>{ed.name}</span>)}</td><td style={{...TD,textAlign:"center"}}><button onClick={()=>setEditors(prev=>prev.map(e=>e.id===ed.id?{...e,role:edRole==="editor"?"crew":"editor"}:e))} style={{padding:"3px 10px",borderRadius:10,border:"none",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",background:edRole==="editor"?"var(--accent-soft)":"rgba(245,158,11,0.15)",color:edRole==="editor"?"var(--accent)":"#F59E0B"}}>{edRole==="editor"?"Editor":"Crew"}</button></td><td style={TD}><input type="text" value={ed.phone||""} onChange={e=>setEditors(prev=>prev.map(x=>x.id===ed.id?{...x,phone:e.target.value}:x))} placeholder="Phone..." style={{width:"100%",padding:"3px 6px",borderRadius:4,border:"1px solid var(--border)",background:"var(--input-bg)",color:"var(--fg)",fontSize:12,outline:"none",fontFamily:"inherit"}}/></td><td style={TD}><input type="text" value={ed.email||""} onChange={e=>setEditors(prev=>prev.map(x=>x.id===ed.id?{...x,email:e.target.value}:x))} placeholder="Email..." style={{width:"100%",padding:"3px 6px",borderRadius:4,border:"1px solid var(--border)",background:"var(--input-bg)",color:"var(--fg)",fontSize:12,outline:"none",fontFamily:"inherit"}}/></td>{DK.map(day=>(<td key={day} onClick={()=>rosterToggle(ed.id,day)} style={{...TD,textAlign:"center",cursor:"pointer",userSelect:"none",background:ed.defaultDays[day]?"var(--accent-soft)":"transparent",color:ed.defaultDays[day]?"var(--accent)":"#3A4558",fontWeight:700}}>{ed.defaultDays[day]?"IN":"-"}</td>))}<td style={{...TD,textAlign:"center",fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{dn}</td><td style={{...TD,textAlign:"center"}}><button onClick={()=>rosterRemove(ed.id)} style={{background:"none",border:"none",cursor:"pointer",color:"#5A6B85",fontSize:16}}>x</button></td></tr>);})}
      {rosterAdding&&(<tr><td style={TD}><input ref={rosterAddRef} type="text" value={rosterNewName} onChange={e=>setRosterNewName(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")rosterAdd();if(e.key==="Escape"){setRosterAdding(false);setRosterNewName("");}}} placeholder="Name..." style={{width:"100%",padding:"5px 8px",borderRadius:6,border:"1px solid var(--accent)",background:"var(--input-bg)",color:"var(--fg)",fontSize:13,fontWeight:600,outline:"none"}}/></td><td style={TD} colSpan={7}></td><td style={{...TD,textAlign:"center"}}><button onClick={rosterAdd} style={{...BTN,background:"var(--accent)",color:"white"}}>Add</button></td><td style={{...TD,textAlign:"center"}}><button onClick={()=>{setRosterAdding(false);setRosterNewName("");}} style={{background:"none",border:"none",cursor:"pointer",color:"#5A6B85",fontSize:16}}>x</button></td></tr>)}</tbody></table></div>{!rosterAdding&&<button onClick={()=>setRosterAdding(true)} style={{marginTop:12,padding:"8px 16px",borderRadius:8,border:"1px dashed var(--border)",background:"transparent",color:"var(--muted)",fontSize:12,fontWeight:600,cursor:"pointer"}}>+ Add Team Member</button>}</div>)}
    ...
    {tool==="deliveries"&&isFounder&&(()=>{
      ...
      const findAcct=(clientName)=>{if(!clientName)return null;const nameLC=clientName.trim().toLowerCase();const acctList=Object.values(accounts).filter(Boolean);const exact=acctList.find(a=>(a.companyName||"").trim().toLowerCase()===nameLC);if(exact)return exact;const partial=acctList.find(a=>{const acn=(a.companyName||"").trim().toLowerCase();return acn&&(acn.includes(nameLC)||nameLC.includes(acn));});return partial||null;};
          ...
          :(<div style={{display:"grid",gap:12}}>
            {deliveries.sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)).map(d=>{
              ...
              return(<div key={d.id} style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:10,padding:"16px 20px",cursor:"pointer",transition:"all 0.15s"}} onClick={()=>setActiveDeliveryId(d.id)}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div style={{display:"flex",alignItems:"center",gap:12}}>
                    {(()=>{const logoSrc=getAcctLogo(d.clientName)||d.logoUrl;const bg=logoBg(getAcctLogoBg(d.clientName));return logoSrc?<img key={logoSrc+bg} src={logoSrc} alt="" onError={e=>{e.target.style.display="none";}} style={{height:28,borderRadius:4,objectFit:"contain",background:bg,padding:3}}/>:null;})()}
                    <div>
