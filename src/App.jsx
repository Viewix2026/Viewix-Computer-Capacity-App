...
// Firebase via CDN compat layer
const FB_URL = "https://www.gstatic.com/firebasejs/10.12.2/firebase-app-compat.js";
const FB_DB_URL = "https://www.gstatic.com/firebasejs/10.12.2/firebase-database-compat.js";
...
function initFirebase() {
  if (fbReady || document.getElementById("fb-app-script")) return;
  const s1 = document.createElement("script");
  s1.id = "fb-app-script"; s1.src = FB_URL;
  s1.onload = () => {
...
function onFbReady(cb) {
  if (fbReady) cb(); else fbReadyCallbacks.push(cb);
}
...
function wKey(md){const d=new Date(md);return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;}
function fmtD(d){return new Date(d).toLocaleDateString("en-AU",{day:"numeric",month:"short"});}
function fmtRange(md){const m=new Date(md);const f=new Date(m);f.setDate(f.getDate()+4);return `${fmtD(m)} - ${fmtD(f)}`;}
function fmtLabel(md){return new Date(md).toLocaleDateString("en-AU",{day:"numeric",month:"short",year:"numeric"});}
function dayDates(md){return DAY_KEYS.map((_,i)=>{const d=new Date(md);d.setDate(d.getDate()+i);return d;});}
...
function Grid({wk,weekData,onUpdate,masterEds,inputs,onUpdateSuites}){
  const md=new Date(wk+"T00:00:00");const dd=dayDates(md);
  const data=weekData[wk]||{};
  const eds=data.editors||masterEds.map(e=>({...e,days:{...e.defaultDays}}));
  ...
  const addRef=useRef(null);const editRef=useRef(null);

  const sv=patch=>onUpdate(wk,{editors:eds,...data,...patch});
  const tog=(eid,day)=>sv({editors:eds.map(e=>e.id===eid?{...e,days:{...e.days,[day]:!e.days[day]}}:e)});
  const doAdd=()=>{if(!newName.trim())return;sv({editors:[...eds,{id:`ed-${Date.now()}`,name:newName.trim(),days:{mon:true,tue:true,wed:true,thu:true,fri:true}}]});setNewName("");setAdding(false);};
  const rmEd=id=>sv({editors:eds.filter(e=>e.id!==id)});
  const startEdit=ed=>{setEditingId(ed.id);setEditName(ed.name);};
  const doRename=()=>{if(!editName.trim()){setEditingId(null);return;}sv({editors:eds.map(e=>e.id===editingId?{...e,name:editName.trim()}:e)});setEditingId(null);setEditName("");};

  const occPerDay=DAY_KEYS.map(d=>eds.filter(e=>e.days[d]).length);
  ...
  return(<div><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"separate",borderSpacing:0,fontSize:13}}>
    <thead><tr><th style={{...TH,width:150,textAlign:"left"}}>Editor</th>{DAY_KEYS.map((_,i)=>(<th key={i} style={{...TH,textAlign:"center",minWidth:90}}><div>{DAY_LABELS[i]}</div><div style={{fontSize:11,fontWeight:600,color:"var(--accent)",marginTop:2}}>{fmtD(dd[i])}</div></th>))}<th style={{...TH,width:55,textAlign:"center"}}>Days</th><th style={{...TH,width:40}}></th></tr></thead>
    <tbody>{eds.map(ed=>{const dn=DAY_KEYS.filter(d=>ed.days[d]).length;const isE=editingId===ed.id;return(<tr key={ed.id}><td style={{...TD,fontWeight:700,color:"var(--fg)",cursor:"pointer"}} onClick={()=>{if(!isE)startEdit(ed);}}>{isE?(<input ref={editRef} type="text" value={editName} onChange={e=>setEditName(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")doRename();if(e.key==="Escape")setEditingId(null);}} onBlur={doRename} style={{width:"100%",padding:"3px 6px",borderRadius:4,border:"1px solid var(--accent)",background:"var(--input-bg)",color:"var(--fg)",fontSize:13,fontWeight:700,outline:"none"}}/>):(<span style={{borderBottom:"1px dashed #3A4558"}} title="Click to rename">{ed.name}</span>)}</td>{DAY_KEYS.map(day=>(<td key={day} onClick={()=>tog(ed.id,day)} style={{...TD,textAlign:"center",cursor:"pointer",userSelect:"none",transition:"all 0.15s",background:ed.days[day]?"var(--accent-soft)":"transparent",color:ed.days[day]?"var(--accent)":"#3A4558",fontWeight:700}}>{ed.days[day]?"IN":"-"}</td>))}<td style={{...TD,textAlign:"center",fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{dn}</td><td style={{...TD,textAlign:"center"}}><button onClick={()=>rmEd(ed.id)} style={{background:"none",border:"none",cursor:"pointer",color:"#5A6B85",fontSize:16,padding:"2px 6px",borderRadius:4}} title={`Remove ${ed.name}`}>x</button></td></tr>);})}
    {adding&&(<tr><td style={TD}><input ref={addRef} type="text" value={newName} onChange={e=>setNewName(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")doAdd();if(e.key==="Escape"){setAdding(false);setNewName("");}}} placeholder="Editor name..." style={{width:"100%",padding:"5px 8px",borderRadius:6,border:"1px solid var(--accent)",background:"var(--input-bg)",color:"var(--fg)",fontSize:13,fontWeight:600,outline:"none"}}/></td><td style={TD} colSpan={5}></td><td style={{...TD,textAlign:"center"}}><button onClick={doAdd} style={{background:"var(--accent)",border:"none",borderRadius:4,color:"white",fontSize:11,fontWeight:700,padding:"4px 10px",cursor:"pointer"}}>Add</button></td><td style={{...TD,textAlign:"center"}}><button onClick={()=>{setAdding(false);setNewName("");}} style={{background:"none",border:"none",cursor:"pointer",color:"#5A6B85",fontSize:16,padding:"2px 6px"}}>x</button></td></tr>)}</tbody>
...
function Login({onLogin}){
  ...
  return(<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"var(--bg)",fontFamily:"'DM Sans',-apple-system,sans-serif"}}><div style={{width:380,padding:"48px 40px",background:"var(--card)",borderRadius:16,border:"1px solid var(--border)",textAlign:"center",animation:shake?"shake 0.5s ease":"none"}}><div style={{marginBottom:32,display:"flex",justifyContent:"center"}}><Logo h={36}/></div><div style={{fontSize:18,fontWeight:700,color:"var(--fg)",marginBottom:6}}>Capacity Planner</div><div style={{fontSize:13,color:"var(--muted)",marginBottom:28}}>Enter password to continue</div><div><input ref={ref} type="password" value={pw} onChange={e=>{setPw(e.target.value);setErr(false);}} onKeyDown={keyDown} placeholder="Password" style={{width:"100%",padding:"12px 16px",borderRadius:10,border:`1px solid ${err?"#EF4444":"var(--border)"}`,background:"var(--input-bg)",color:"var(--fg)",fontSize:15,outline:"none",marginBottom:12,textAlign:"center",letterSpacing:"0.15em"}}/>{err&&<div style={{fontSize:12,color:"#EF4444",marginBottom:10}}>Wrong password</div>}<button onClick={go} style={{width:"100%",padding:"12px",borderRadius:10,border:"none",background:"#0082FA",color:"white",fontSize:14,fontWeight:700,cursor:"pointer"}}>Sign In</button></div></div></div>);
}

// ─── Main App with Firebase ───
export default function App(){
  ...
  // Track whether data came from Firebase to avoid writing defaults back
  const fbLoaded=useRef(false);
  const skipWrite=useRef(true);

  // Init Firebase and listen for data
  useEffect(()=>{
    initFirebase();
    onFbReady(()=>{
      fbListen("/",(data)=>{
        ...
        setLoading(false);
        // Allow writes after first load
        setTimeout(()=>{skipWrite.current=false;},500);
      });
    });
  },[]);

  // Write to Firebase on changes (debounced)
  ...
  useEffect(()=>{
    ...
    writeTimer.current=setTimeout(()=>{
      fbSet("/inputs",inputs);
      fbSet("/editors",editors);
      fbSet("/weekData",weekData);
    },300);
  },[inputs,editors,weekData]);
  ...
  const login=pw=>{if(pw===PW){setAuthed(true);return true;}return false;};
  ...
  const rosterRename=()=>{if(!rosterEditName.trim()){setRosterEditId(null);return;}setEditors(prev=>prev.map(e=>e.id===rosterEditId?{...e,name:rosterEditName.trim()}:e));setRosterEditId(null);setRosterEditName("");};

  const ai=scMode&&scIn?scIn:inputs;
  const cwEds=weekData[curW]?.editors||editors.map(e=>({...e,days:{...e.defaultDays}}));
  const occ=cwEds.reduce((s,e)=>s+DAY_KEYS.filter(d=>e.days[d]).length,0);
  ...
  if(!authed)return(<><style>{CSS}</style><Login onLogin={login}/></>);

  if(loading)return(<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#0B0F1A",fontFamily:"'DM Sans',sans-serif"}}><style>{CSS}</style><div style={{textAlign:"center"}}><Logo h={36}/><div style={{marginTop:16,color:"#5A6B85",fontSize:14}}>Loading...</div></div></div>);

  return(<div style={{fontFamily:"'DM Sans',-apple-system,sans-serif",background:"var(--bg)",color:"var(--fg)",minHeight:"100vh"}}><style>{CSS}</style>
    <div style={{padding:"14px 28px",borderBottom:"1px solid var(--border)",display:"flex",alignItems:"center",justifyContent:"space-between",background:"var(--card)",flexWrap:"wrap",gap:12}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}><Logo h={28}/><div style={{width:1,height:24,background:"var(--border)"}}/><span style={{fontSize:13,fontWeight:600,color:"var(--muted)",letterSpacing:"0.02em"}}>Capacity Planner</span></div>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <div style={{display:"flex",gap:3,background:"var(--bg)",borderRadius:8,padding:3}}>
          {[{key:"dashboard",label:"Dashboard"},{key:"roster",label:"Team Roster"},{key:"schedule",label:"Weekly Schedule"},{key:"forecast",label:"Forecast"}].map(t=>(<button key={t.key} onClick={()=>setTab(t.key)} style={{padding:"7px 14px",borderRadius:6,border:"none",background:tab===t.key?"var(--card)":"transparent",color:tab===t.key?"var(--fg)":"var(--muted)",fontSize:12,fontWeight:600,cursor:"pointer",transition:"all 0.15s"}}>{t.label}</button>))}
        </div>
        <button onClick={()=>setAuthed(false)} style={{padding:"5px 12px",borderRadius:6,border:"1px solid var(--border)",background:"transparent",color:"var(--muted)",fontSize:11,fontWeight:600,cursor:"pointer"}}>Log Out</button>
      </div>
    </div>

    {scMode&&(<div style={{padding:"10px 28px",background:"#1A1510",borderBottom:"1px solid #3D2E10",display:"flex",alignItems:"center",justifyContent:"space-between"}}><span style={{fontSize:12,fontWeight:700,color:"#F59E0B"}}>WHAT-IF MODE - Changes won't save until you apply</span><div style={{display:"flex",gap:8}}><button onClick={()=>{if(scIn)setInputs(scIn);setScMode(false);setScIn(null);}} style={{padding:"5px 14px",borderRadius:6,border:"none",background:"#10B981",color:"white",fontSize:11,fontWeight:700,cursor:"pointer"}}>Apply</button><button onClick={()=>{setScMode(false);setScIn(null);}} style={{padding:"5px 14px",borderRadius:6,border:"none",background:"#374151",color:"#9CA3AF",fontSize:11,fontWeight:700,cursor:"pointer"}}>Discard</button></div></div>)}
    ...
    {tab==="forecast"&&(<>
      <div style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:12,padding:"20px 24px",marginBottom:20}}><div style={{fontSize:13,fontWeight:700,color:"var(--fg)",marginBottom:4}}>12-Week Workload Forecast</div><div style={{fontSize:12,color:"var(--muted)",marginBottom:16}}>{ai.newProjectsPerWeek} new/week, {ai.avgProjectDuration}-week duration, {ai.avgEditHoursPerProject}h avg edit</div><FChart forecast={c.forecast}/></div>
      <div style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:12,padding:"20px 24px",overflowX:"auto"}}><div style={{fontSize:13,fontWeight:700,color:"var(--fg)",marginBottom:12}}>Forecast Detail</div><table style={{width:"100%",borderCollapse:"separate",borderSpacing:0,fontSize:12}}><thead><tr>{["Week","Projects","Workload","Real Util","Filled Util","Suites Needed","Status"].map(h=>(<th key={h} style={{...TH,textAlign:h==="Week"?"left":"center"}}>{h}</th>))}</tr></thead><tbody>{c.forecast.map(f=>(<tr key={f.week}><td style={{...TD,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>W{f.week}{f.week===0?" (now)":""}</td><td style={{...TD,textAlign:"center",fontFamily:"'JetBrains Mono',monospace"}}>{f.projects}</td><td style={{...TD,textAlign:"center",fontFamily:"'JetBrains Mono',monospace"}}>{f.workload}h</td><td style={{...TD,textAlign:"center"}}><div style={{display:"flex",alignItems:"center",gap:8,justifyContent:"center"}}><div style={{width:60}}><UBar value={f.realUtil} height={8}/></div><span style={{fontFamily:"'JetBrains Mono',monospace",fontWeight:700,fontSize:11,color:f.realUtil>=0.95?"#EF4444":"var(--fg)"}}>{pct(f.realUtil)}</span></div></td><td style={{...TD,textAlign:"center",fontFamily:"'JetBrains Mono',monospace"}}>{pct(f.filledUtil)}</td><td style={{...TD,textAlign:"center",fontFamily:"'JetBrains Mono',monospace",fontWeight:700}}>{f.suitesNeeded}</td><td style={{...TD,textAlign:"center"}}><Badge util={f.realUtil}/></td></tr>))}</tbody></table></div>
    </>)}
