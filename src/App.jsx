      ...
      fbListen("/",(data)=>{
        try{
          if(data){
            ...
            if(data.founderTodos&&Array.isArray(data.founderTodos)){
              setFounderTodos(data.founderTodos);
            }
            if(data.teamLunch){
    ...
    {tool==="capacity"&&isFounder&&(<>
      <div style={{padding:"12px 28px",borderBottom:"1px solid var(--border)",display:"flex",alignItems:"center",justifyContent:"space-between",background:"var(--card)"}}>
        ...
        <div style={{display:"flex",gap:3,background:"var(--bg)",borderRadius:8,padding:3}}>
          {[{key:"dashboard",label:"Dashboard"},{key:"roster",label:"Team Roster"},{key:"schedule",label:"Weekly Schedule"},{key:"forecast",label:"Forecast"},{key:"timelogs",label:"Time Logs"},{key:"todos",label:"To Do"}].map(t=>(<button key={t.key} onClick={()=>setCapTab(t.key)} style={{padding:"7px 14px",borderRadius:6,border:"none",background:capTab===t.key?"var(--card)":"transparent",color:capTab===t.key?"var(--fg)":"var(--muted)",fontSize:12,fontWeight:600,cursor:"pointer"}}>{t.label}</button>))}
        </div>
      </div>
    ...
    {tool==="founders"&&isFounders&&(()=>{
      ...
      return(<>
        ...
        <div style={{maxWidth:1200,margin:"0 auto",padding:"24px 28px 60px"}}>
          ...
          {/* Team Lunch Manager */}
          <div style={{marginBottom:20,padding:"20px 24px",background:"var(--card)",border:"1px solid var(--border)",borderRadius:12}}>
            <div style={{fontSize:13,fontWeight:700,color:"var(--fg)",marginBottom:12}}>Set Next Team Lunch</div>
            <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
              ...
              <input type="text" value={teamLunch?.time||""} onChange={e=>setTeamLunch(p=>({...p,time:e.target.value}))} placeholder="Time (e.g. 12:30pm)" style={{padding:"8px 12px",borderRadius:8,border:"1px solid var(--border)",background:"var(--input-bg)",color:"var(--fg)",fontSize:13,outline:"none",width:160}}/>
              <input type="text" value={teamLunch?.location||""} onChange={e=>setTeamLunch(p=>({...p,location:e.target.value}))} placeholder="Location" style={{padding:"8px 12px",borderRadius:8,border:"1px solid var(--border)",background:"var(--input-bg)",color:"var(--fg)",fontSize:13,outline:"none",flex:1,minWidth:180}}/>
            </div>
            <input type="text" value={teamLunch?.notes||""} onChange={e=>setTeamLunch(p=>({...p,notes:e.target.value}))} placeholder="Notes (optional)" style={{marginTop:8,width:"100%",padding:"8px 12px",borderRadius:8,border:"1px solid var(--border)",background:"var(--input-bg)",color:"var(--fg)",fontSize:13,outline:"none"}}/>
          </div>
