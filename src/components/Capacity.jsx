                  ...
                  {[...editors].sort((a,b)=>(a?.name||"").toLowerCase().localeCompare((b?.name||"").toLowerCase())).map(ed => {
                    ...
                    return (
                      <tr key={ed.id} style={{ opacity: edRole === "crew" ? 0.75 : 1 }}>
                        ...
                        <td style={{ ...TD, textAlign: "center" }}>
                          <button onClick={() => setEditors(prev => prev.map(e => e.id === ed.id ? { ...e, role: edRole === "editor" ? "crew" : "editor" } : e))} title={edRole === "editor" ? "Occupies an edit suite — counted in weekly schedule" : "No edit suite — hidden from weekly schedule"} style={{ padding: "3px 10px", borderRadius: 10, border: "none", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", background: edRole === "editor" ? "rgba(34,197,94,0.15)" : "rgba(90,107,133,0.15)", color: edRole === "editor" ? "#22C55E" : "var(--muted)" }}>{edRole === "editor" ? "✓ Yes" : "— No"}</button>
                        </td>
                        ...
                        <td style={{ ...TD, textAlign: "center" }}><button onClick={() => rosterRemove(ed.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--muted)", fontSize: 16 }}>x</button></td>
                      </tr>
                    );
                  })}
                  {rosterAdding && (
                    <tr>
                      ...
                      <td style={{ ...TD, textAlign: "center" }}><button onClick={() => { setRosterAdding(false); setRosterNewName(""); }} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--muted)", fontSize: 16 }}>x</button></td>
                    </tr>
                  )}
                </tbody>
              </table>
