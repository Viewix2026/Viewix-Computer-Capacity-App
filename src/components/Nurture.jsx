// Nurture tab — Lapsed-Proposal Auto-Recovery System
//
// V1 = read-only view. The full layout from the plan ships, but the
// automation columns (in-sequence, replied, delivered, channel attribution,
// deferred queue) are intentionally empty until the GHL/Aimfox/classifier
// workers land. The tab reads:
//   - /attioCache from Firebase (deals + companies, refreshed nightly by
//     api/sync-attio-cache.js, also triggered manually from Founders)
...
import { fbListen, fbSet } from "../firebase";
...
export function Nurture({ attioDeals, isFounder }) {
  ...
  const rows = useMemo(() => {
    return deals.map((d) => {
      ...
      if (cs?.status) {
        ...
        status = qAt ? "Reconverted" : null; // only counts if it was Quoted first
      } else {
        status = null;
      }
      return {
    ...
    <>
      ...
      <div style={{ padding: "12px 28px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", background: "var(--card)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <span style={{ fontSize: 15, fontWeight: 700, color: "var(--fg)" }}>Nurture · Lapsed-Proposal Recovery</span>
