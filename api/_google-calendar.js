...
//
// This module's exports (createShootEvent / updateShootEvent /
// deleteShootEvent / getEventById / getCalendarClient) are the reuse
// surface for any future calendar work (e.g. internal-review invites).
...
export async function updateShootEvent({ eventId, project, subtask, attendees }) {
  ...
  // events.patch — attendees array REPLACES, not merges. Caller must
  // pass the FULL intended list every time.
...
export async function deleteShootEvent({ eventId, sendUpdates = "all" }) {
  ...
  const cal = getCalendarClient();
  try {
    await cal.events.delete({ calendarId: calendarId(), eventId, sendUpdates });
    return { ok: true };
  } catch (e) {
