...
// Splitting into three explicit outcomes is the fix for the
...
//
// Shoot definition is STRICT (Jeremy's call, 2026-05-27): only
// stage==="shoot" syncs. The Selects engine uses a looser name-match,
// but calendar sync deliberately does not.
export function getCalendarSyncDecision({ subtask, project }) {
  ...
  // ── HOLD-ERROR paths (event stays, warning surfaces) ──────────────
  // E1 (Jeremy, 2026-05-27): empty crew is HOLD, not delete. Removing
  // all crew is ambiguous (could be a mid-reshuffle), so we keep the
  // existing event LIVE and surface a loud warning rather than firing
  // a cancellation email. The producer cancels deliberately by
  // unscheduling or toggling sync off.
  if (!Array.isArray(subtask.assigneeIds) || subtask.assigneeIds.length === 0) {
    return {
      action: "hold-error",
      message: "No crew assigned — calendar event is still live. Add crew, or unschedule / toggle sync off to remove it.",
    };
  }
