...
// Run with `npm run test:calendar` — uses Node's built-in
// `node:test`, no external runner needed.
...
test("eventIdFor is deterministic for the same inputs", () => {
  assert.equal(eventIdFor("proj-1", "sub-1"), eventIdFor("proj-1", "sub-1"));
});
...
test("combineDateTimeSydney does NOT call new Date()", () => {
  ...
  let invoked = false;
  function StubDate(...args) { invoked = true; return new realDate(...args); }
...
const validProject = { id: "p1", clientContact: { email: "client@example.com" } };

test("decision: valid shoot → sync", () => {
  assert.equal(getCalendarSyncDecision({ subtask: validSubtask, project: validProject }).action, "sync");
});
...
test("decision: syncToCalendar=false → delete (toggle-off)", () => {
  const d = getCalendarSyncDecision({ subtask: { ...validSubtask, syncToCalendar: false }, project: validProject });
  assert.equal(d.action, "delete");
  assert.equal(d.reason, "toggle-off");
});

test("decision: stage moved away from shoot → delete", () => {
  const d = getCalendarSyncDecision({ subtask: { ...validSubtask, stage: "edit" }, project: validProject });
  assert.equal(d.action, "delete");
...
test("decision: dates cleared → delete (unscheduled)", () => {
  const d = getCalendarSyncDecision({ subtask: { ...validSubtask, startDate: null, endDate: null }, project: validProject });
  assert.equal(d.action, "delete");
  assert.equal(d.reason, "unscheduled");
});

// E1 (Jeremy, 2026-05-27): empty crew HOLDS, does NOT delete/cancel.
test("decision: empty assigneeIds → hold-error (NOT delete) — E1", () => {
  const d = getCalendarSyncDecision({ subtask: { ...validSubtask, assigneeIds: [] }, project: validProject });
  assert.equal(d.action, "hold-error");
  assert.match(d.message, /no crew assigned/i);
  assert.match(d.message, /still live/i);
});

test("decision: missing assigneeIds field → hold-error — E1", () => {
  const st = { ...validSubtask };
  delete st.assigneeIds;
  const d = getCalendarSyncDecision({ subtask: st, project: validProject });
  assert.equal(d.action, "hold-error");
  assert.match(d.message, /no crew assigned/i);
});

test("decision: missing times → hold-error (NOT delete)", () => {
  const d = getCalendarSyncDecision({ subtask: { ...validSubtask, startTime: null, endTime: null }, project: validProject });
  assert.equal(d.action, "hold-error");
...
test("decision: end before start same-day → hold-error", () => {
  const d = getCalendarSyncDecision({
    subtask: { ...validSubtask, startDate: "2026-06-01", startTime: "17:00", endDate: "2026-06-01", endTime: "10:00" },
...
test("decision: multi-day shoot (next-day end) → sync", () => {
  const d = getCalendarSyncDecision({
    subtask: { ...validSubtask, startDate: "2026-06-01", startTime: "17:00", endDate: "2026-06-02", endTime: "10:00" },
    project: validProject,
  });
...
test("decision: missing client email → hold-error (NOT delete)", () => {
  const d = getCalendarSyncDecision({ subtask: validSubtask, project: { id: "p1", clientContact: {} } });
  assert.equal(d.action, "hold-error");
